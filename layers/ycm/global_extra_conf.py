...
def PathsToAllParentFolders( path ):
  folder = p.normpath( path )
  if p.isdir( folder ):
    yield folder
  while True:
    parent = p.dirname( folder )
    if parent == folder:
      break
    folder = parent
    yield folder
...
def FindCompilationDatabase( filename, compilation_database_folder ):
  if p.isabs( compilation_database_folder ):
    return compilation_database_folder

  for folder in PathsToAllParentFolders( filename ):
    compile_commands = p.join( folder, compilation_database_folder,
                               'compile_commands.json')
    if p.exists( compile_commands ):
      return p.dirname( compile_commands )
  return None


def Settings( **kwargs ):
  # Set compilation_database_folder from vim file, you can set the variable
  # g:ycm_compilation_database_folder to a relative path.
  # Refer to https://github.com/ycm-core/YouCompleteMe/issues/2741#issuecomment-320104642
  if not setbypythonfile:
